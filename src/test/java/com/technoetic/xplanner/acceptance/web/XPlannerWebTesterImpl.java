 ...
 * Time: 3:46:09 PM
 */
public class XPlannerWebTesterImpl extends net.sourceforge.jwebunit.WebTester implements XPlannerWebTester {
   ...
   private String extractCellValue(String tableId, String columnName, int rowIndex) {
      ...
      return objectTable.getCellAsText(rowIndex, col);
   }
   ...
   private int[] getRowNumbersWithText(WebTable objectTable, String text) {
      List rowNbrList = new ArrayList();
      for (int row = 0; row < objectTable.getRowCount(); row++) {
         for (int column = 0; column < objectTable.getColumnCount(); column++) {
            if (objectTable.getCellAsText(row, column).indexOf(text) != -1) {
               rowNbrList.add(new Integer(row));
            }
         }
      }
   ...
   private String[] collectCellValues(WebTable objectTable, int[] rows, int col) {
      String[] cellValues = new String[rows.length];
      for (int i = 0; i < cellValues.length; i++) {
         cellValues[i] = objectTable.getCellAsText(rows[i], col);
      }
...
//DEBT Use mail tester directly
   ...
   public void assertOnTopPage() {
      ...
      if (getDialog().isTextInResponse(getMessage("projects.tableheading.name"))) {
   ...
   public void assertOptionListed(String selectName, String option) {
      ...
      String[] allOptions = getDialog().getOptionsFor(selectName);
      for (int i = 0; i < allOptions.length; i++) {
         String singleOption = allOptions[i];
         if (singleOption.compareTo(option) == 0) return;
      }
      Assert.fail("Option not found [" + option + "]");
   }
   ...
   public void assertOptionNotListed(String selectName, String option) {
      ...
      for (int i = 0; i < allOptions.length; i++) {
         String singleOption = allOptions[i];
         Assert.assertTrue("Option found [" + option + "]",
                           singleOption.compareTo(option) != 0);
      }
   }
   ...
   public int getFirstColNumberWithText(WebTable table, int row, String columnName) {
      int colIdx = -1;
   ...
   public int[] getRowNumbersWithText(String tableId, String text) {
      WebResponse editPage = this.getDialog().getResponse();
      WebTable objectTable = getTable(editPage, tableId);
      return getRowNumbersWithText(objectTable, text);
   }

   private WebTable getTable(WebResponse editPage, String tableId) {
      WebTable objectTable;
      try {
         objectTable = editPage.getTableWithID(tableId);
      }
      catch (SAXException e) {
         throw new RuntimeException(e);
      }
      Assert.assertNotNull("Acceptance tests require table with id = " + tableId, objectTable);
      return objectTable;
   }
   ...
   public Session getSession() throws HibernateException {
      Logger.getLogger("net.sf.hibernate").setLevel(Level.WARN);
