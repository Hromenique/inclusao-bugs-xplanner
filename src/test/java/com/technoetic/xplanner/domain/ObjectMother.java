...
public class ObjectMother {
  ...
  public Project newProject() throws HibernateException, RepositoryException {
   ...
   public TimeEntry newTimeEntry(Task task, Person person1, double durationInHours) throws HibernateException,
      ...
      timeEntry.setTaskId(task.getId());
   ...
   public HistoricalEvent newHistoricalEvent() throws HibernateException, RepositoryException {
      ...
      String type = "project";
      String action = HistoricalEvent.DELETED;
      return newHistoricalEvent(targetObjectId, type, action, "PROJECT_NAME");
   }

   public HistoricalEvent newHistoricalEvent(int targetObjectId, String type, String action, String description)
         throws HibernateException, RepositoryException {
      return newHistoricalEvent(0, targetObjectId, type, action, description);
   }

   public HistoricalEvent newHistoricalEvent(int containerId,
                                             int targetObjectId,
                                             String type,
                                             String action,
                                             String description)
         throws HibernateException, RepositoryException {
      HistoricalEvent historyEvent = new HistoricalEvent(containerId, targetObjectId, type, action, description,
                                                         XPlannerTestSupport.DEFAULT_PERSON_ID, getNow());
      save(historyEvent);
      return historyEvent;
   }
   ...
   public void setUpPersonRole(final Project project, final Person person, final String roleName) throws Exception {
     CheckedExceptionHandlingTransactionTemplate transactionTemplate = (CheckedExceptionHandlingTransactionTemplate) MainBeanFactory.getBean("transactionTemplate");
     transactionTemplate.execute(new Callable() {
       public Object run() throws Exception {
         EditPersonHelper editPersonHelper = (EditPersonHelper) MainBeanFactory.getBean("editPersonHelper");
         editPersonHelper.setRoleOnProject(project.getId(), person, roleName);
         return null;
       }
     });
