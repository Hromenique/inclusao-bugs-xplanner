...
import com.technoetic.xplanner.charts.DataSample;
...
public class XPlannerTestSupport {
    ...
    public void assertHistoricalEventInObject(int objectId, String action, String description, int personId) {
        assertHistoricalEvent(hibernateSession, objectId, action, description, personId);
    }

    public void assertHistoricalEvent(MockSession hibernateSession,
                                      int objectId,
                                      String action,
                                      String description,
                                      int authorId) {

        List eventList = getHistoricalEventList(hibernateSession, objectId, action);
        Assert.assertFalse("no historical events", eventList.size() == 0);
        if (eventList.size() > 0) {
            Iterator it = eventList.iterator();
            while (it.hasNext()) {
                HistoricalEvent event = (HistoricalEvent) it.next();
   ...
   private boolean matchesExpectedEvent(String description, HistoricalEvent event, int authorId) {
      return
   ...
   private boolean assertEventDescriptionMatches(String expectedDescription, HistoricalEvent event) {
      return (event.getDescription() == null && expectedDescription == null) ||
             event.getDescription().equals(expectedDescription);
   }

   private void assertEventDetails(String description, HistoricalEvent event, int authorId) {
      Assert.assertEquals("wrong description", description, event.getDescription());
      Assert.assertEquals("wrong authorId", authorId, event.getPersonId());
   ...
   public void assertNoHistoricalEvent(MockSession hibernateSession) {
       List historicalEventList = getHistoricalEventList(hibernateSession, 0, null);
       Assert.assertFalse("unexpected historical event", historicalEventList.size() == 0);
   }

    private List getHistoricalEventList(MockSession hibernateSession, int targetObjectId, String action) {
        List historicalEventList = new ArrayList();
        Iterator it = getObjectListWithType(hibernateSession, HistoricalEvent.class).iterator();
        while (it.hasNext()) {
            HistoricalEvent event = (HistoricalEvent) it.next();
            if (targetObjectId == 0 || (event.getTargetObjectId() == targetObjectId &&
