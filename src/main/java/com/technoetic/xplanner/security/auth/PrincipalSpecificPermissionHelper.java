...
public class PrincipalSpecificPermissionHelper {
   ...
   public Map getPermissionsForPrincipal(int principalId) {
      Map map = new HashMap();
   ...
   void addPersonPermissions(
         int principalId,
         Map map) {
      Collection permissionsLists = new ArrayList(map.values());
      for (Iterator it = permissionsLists.iterator(); it.hasNext();) {
         List list = new ArrayList((List) it.next());
         for (Iterator itPerm = list.iterator(); itPerm.hasNext();) {
            Permission p = (Permission) itPerm.next();
   ...
   void addPermissionForProject(Map permissionMap, int projectId, Permission permission) {
      Integer projectKey = new Integer(projectId);
      List permissions = (List) permissionMap.get(projectKey);
      if (permissions == null) {
