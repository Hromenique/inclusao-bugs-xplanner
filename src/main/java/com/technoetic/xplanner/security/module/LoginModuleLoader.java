...
public class LoginModuleLoader {
   ...
   public LoginModule[] loadLoginModules() throws ConfigurationException
   {
      ...
      while (true) {
         ...
         if (loginModuleClassName == null) break;

         String loginModuleName = properties.getProperty(getKey(LOGIN_MODULE_NAME_KEY, idx));
         if (loginModuleName == null)
         {
            throw new ConfigurationException(LoginModule.MESSAGE_NO_MODULE_NAME_SPECIFIED_ERROR_KEY);
         }
         HashMap options = getOptions(properties, idx);
   ...
   private LoginModule createModule(String loginModuleClassName,
                                    String loginModuleName,
                                    HashMap options) {
      LoginModule loginModule;
      try {
         Object bean = MainBeanFactory.getBean(loginModuleClassName);
   ...
   private HashMap getOptions(XPlannerProperties properties, int idx) {
      ...
      while (propertyNames.hasNext())
      {
          ...
          String optionName = MessageFormat.format(LOGIN_OPTION_PREFIX, new Integer[]{new Integer(idx)});
   ...
   public static String[] getLoginModuleNames()
   {
      ...
      {
         loginModuleNameList.add(
             properties.getProperty(getKey(LOGIN_MODULE_NAME_KEY, i)));
      }
