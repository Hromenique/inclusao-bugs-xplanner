...
public class TwikiTag extends TagSupport {
   ...
   public int doEndTag() throws JspException {
      try {
         ...
         Properties properties =
               new DomainSpecificPropertiesFactory(HibernateHelper.getSessionFactory(),
                                                                           XPlannerProperties.getProperties()).createPropertiesFor(obj);
         WikiFormat formatter = getFormatter(properties);
         if (content != null) {
            formatter.setProperties(properties);
            pageContext.getOut().print(formatter.format(content.toString()));
         }
      } catch (Exception ex) {
   ...
   private synchronized WikiFormat getFormatter(Properties properties) {
      if (formatter == null) {
         ...
         if (properties.getProperty("wiki.format") != null) {
            try {
               formatter = (WikiFormat) Class.forName(properties.
         ...
         if (key.startsWith(prefix)) {
            ...
            if (key.endsWith(handlerSuffix) &&
               ...
               try {
                  ...
                  try {
                     Constructor argConstructor = handlerClass.getConstructor(
                           new Class[]{String.class});
                     if (argConstructor != null) {
                        handler =
                              (SchemeHandler) argConstructor.newInstance(new Object[]{argument});
                     }
                  } catch (Exception e) {
                     handler = (SchemeHandler) handlerClass.newInstance();
                  }
