...
public class DataSamplerImpl extends HibernateDaoSupport implements DataSampler {
   ...
   public static final String DATASAMPLE_FOR_OBJECT_IN_TIME_QUERY =
         "com.technoetic.xplanner.domain.DataSamplesForObjectInTime";
   public static final String IS_TODAY_SAMPLE_ALREADY_GENERATED_QUERY =
         "com.technoetic.xplanner.domain.IsTodaySampleAlreadyGenerated";
   public static final String GET_DATASAMPLE_QUERY =
         "com.technoetic.xplanner.domain.GetDataSample";
   ...
   public void generateDataSamples(Iteration iteration) {
      ...
      Date tomorrowMidnight = TimeGenerator.shiftDate(todayMidnight, Calendar.DATE, 1);
   ...
   protected void extendIterationEndDateIfNeeded(Iteration iteration, Date midnight) {
      boolean automaticallyExtendEndDate =
            Boolean.valueOf(properties.getProperty(AUTOMATICALLY_EXTEND_END_DATE_PROP, "false")).booleanValue();
      if (automaticallyExtendEndDate &&
          IterationStatus.ACTIVE.equals(iteration.getStatus()) &&
   ...
   protected void saveSamples(Date date, Iteration iteration) {
      saveSample(date, iteration, "estimatedHours", iteration.getEstimatedHours());
      saveSample(date, iteration, "actualHours", iteration.getCachedActualHours());
      saveSample(date, iteration, "remainingHours", iteration.getTaskRemainingHours());
   }

   protected void saveSample(Date date, Iteration iteration, String aspect, double value) {
      DataSample sample;
      List samples = getHibernateOperations().findByNamedQuery(GET_DATASAMPLE_QUERY,
                                                               new Object[]{date,
                                                               ...
                                                               new Type[]{Hibernate.DATE,
                                                                          ...
                                                                          Hibernate.STRING});
      if (!samples.isEmpty()) {
         sample = (DataSample) samples.get(0);
         sample.setValue(value);
         getHibernateOperations().update(sample);
         LOG.debug("update existing datasample");
      } else {
         sample = new DataSample(date, iteration.getId(), aspect, value);
         getHibernateOperations().save(sample);
