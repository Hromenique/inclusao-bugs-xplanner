...
{
   public IdSearchHelper idSearchHelper = new IdSearchHelper();
   private DomainMetaDataRepository metaDataRepository;

   protected ActionForward doExecute(ActionMapping mapping, ActionForm form,
                                     HttpServletRequest request, HttpServletResponse response) throws Exception
   {
      AbstractEditorForm.initConverters(request); // DEBT(SPRING) Extract the convert methods out of the AbstractEditorForm (keep delegators methods) and have that object spring loaded using the message bundle from spring
      ...
      if (integer == null) return getGeneralErrorForward(mapping, request, "idsearch.error.badId", searchedIdString);
      ...
      //DEBT(SPRING): DomainMetaDataRep should be made an instance and spring injected
