...
<db:useBean id="story" type="com.technoetic.xplanner.domain.UserStory" scope="request" />
    ...
    <tr>
      ...
      <tr>
        ...
        <th nowrap><bean:message key="story.label.status"/> </th>
  		  <td><%=messages.getMessage(story.getStatus().getNameKey())%></td>
		</tr>
 </table>
</div>
</tiles:put>
...
<xplanner:writableTable  wholeCollection='<%=story.getTasks() %>' permissions="edit,delete"
    ...
    <bean:define id="task" name="objecttable" type="com.technoetic.xplanner.domain.Task"/>
    ...
    <dt:column sortable="true" sortProperty="acceptorName" title='<%= messages.getMessage("tasks.tableheading.acceptor")%>'>
        <logic:notEqual name="task" property="acceptorId" value="0">
            <db:useBean
                id="acceptor" type="com.technoetic.xplanner.domain.Person"
    ...
    <dt:column align="center" sortable="true" title='<%= messages.getMessage("tasks.tableheading.disposition")%>'>
        <%=messages.getMessage(task.getDisposition().getNameKey())%>
    </dt:column>
...
<xplanner:actionButtons
    ...
    <xplanner:isUserAuthorized name="story" permission='<%=action.getPermission()%>'>
      <xplanner:link page='<%=action.getTargetPage()%>' paramId="oid" onclick='<%=action.getOnclick()%>' fkey='<%=story.getIterationId()%>'
...
<tiles:put name="globalActions" direct="true">
 ...
 <xplanner:link page="/do/view/history">
    ...
    <xplanner:linkParam id='<%= AbstractAction.TYPE_KEY %>' value='<%= story.getClass().getName() %>'/>
 </xplanner:link>
</tiles:put>
