...
<%@ page import="com.technoetic.xplanner.security.auth.SystemAuthorizer,
                 ...
                 com.technoetic.xplanner.domain.Project,
...
<xplanner:writableTable wholeCollection='<%= projects %>' permissions="admin.edit,sysadmin.delete"
    name="pageScope.projects" id="objecttable" styleClass="objecttable" requestURI=""
     >
  <% final Project project = (Project)objecttable; pageContext.setAttribute("project", objecttable); %>
  <xplanner:actionButtonsColumn
        name="project" id="action" title='<%= messages.getMessage("iterations.tableheading.actions") %>' >
  ...
  <xplanner:link href="project" paramId="oid" paramName="project" paramProperty="id">
    <bean:write name="project" property="id"/></xplanner:link></dt:column>
  <dt:column title='<%= messages.getMessage("projects.tableheading.name") %>' sortable="true" sortProperty="name">
    <xplanner:link styleClass="name"  href="project" paramId="oid" paramName="project" paramProperty="id">
    <bean:write name="project" property="name"/></xplanner:link></dt:column>
  <dt:column title='<%= messages.getMessage("projects.tableheading.iteration") %>' sortable="true" sortProperty="name">
  <%
      pageContext.removeAttribute("iteration");
      Iteration iteration = (Iteration)CollectionUtils.find(iterations, new Predicate() {
          public boolean evaluate(Object o) {
              return ((Iteration)o).getProjectId() == project.getId();
          }
      });
      if (iteration != null) {
    ...
    <logic:present name="iteration">
      <xplanner:link href="iteration" paramId="oid" paramName="iteration" paramProperty="id">
        <bean:write name="iteration" property="name"/></xplanner:link>
    </logic:present></dt:column>
  <xplanner:isUserAuthorizedForAny name="projects" permissions="hide">
      <dt:column title='<%= messages.getMessage("projects.tableheading.ishidden") %>'
...
<logic:equal name="projectCount" value="0" >
    <span class="highlighted_message"><bean:message key="projects.none"/></span>
</logic:equal>


<span class="links">
<xplanner:isUserAuthorized resourceType="system.project" permission="create.project">
    <xplanner:link page="/do/edit/project"><bean:message key="projects.link.add_project"/></xplanner:link> 
</xplanner:isUserAuthorized>
</span>
